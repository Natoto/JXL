...
-(void)setcellProfile:(NSString *)profile
{
    if ([profile isUrl]) {//如果是网络图片 就加载网络图片
        [self.imageView sd_setImageWithURL:[NSURL URLWithString:profile] placeholderImage:[UIImage imageNamed:@"defaultprofile"] options:SDWebImageLowPriority completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
-(void)setcellpicturecolor:(NSString *)picturecolor
{
    if (picturecolor) {
        if (self.imageView.image) {
            if ([picturecolor isEqualToString:@"gray"]) {
                self.imageView.image = [self.imageView.image imageWithTintColor:[UIColor grayColor]];
            }
            if ([picturecolor isEqualToString:@"white"]) {
                self.imageView.image = [self.imageView.image imageWithTintColor:[UIColor whiteColor]];
            }
        }
    }
}
...
{
     UIColor * titlecolor = [JXL_Common colorWithStructKey:color] ;
    if (titlecolor) {
